...
export class AuthService {
  ...
  createCharacter(character){
    let headers = new Headers();
    headers.append('Content-Type', 'application/json');
    return this.http.post('http://localhost:3001/characters/create', character,{headers: headers})
      .map(res => res.json());
  }
  ...
  getCharactersByUser(owner){
    let headers = new Headers();
    headers.append('Content-Type', 'application/json');
    return this.http.post('http://localhost:3001/characters/'+owner, {headers: headers})
      .map(res => res.json());
  }  
